...
// ── Lazy-load the two new heavy panels ────────────────────────────────────────
...
function PanelLoader({ label }: { label: string }) {
  return (
    <div style={{ padding: 48, display: "flex", alignItems: "center", justifyContent: "center", gap: 12, color: "#7A9BBF", fontFamily: "monospace", fontSize: 13 }}>
...
// ─── Tab definition ───────────────────────────────────────────────────────────
type TabId = "chart" | "ai" | "top15" | "mystocks";

interface TabDef { id: TabId; label: string; icon: React.ReactNode }

const TABS: TabDef[] = [
  { id:"chart",    label:"Dashboard",  icon:<Zap    size={13}/> },
  { id:"ai",       label:"AI Picks",   icon:<Brain  size={13}/> },
  { id:"top15",    label:"Top 15",     icon:<Trophy size={13}/> },
  { id:"mystocks", label:"My Stocks",  icon:<BookOpen size={13}/> },
];
...
function seedBars(basePrice: number, days = 90): Bar[] {
  ...
  for (let i = days; i >= 0; i--) {
    ...
    const o = p, c = p + chg;
    bars.push({ date:d.toISOString().split("T")[0], open:+o.toFixed(2), high:+(Math.max(o,c)*(1+rand()*0.008)).toFixed(2), low:+(Math.min(o,c)*(1-rand()*0.008)).toFixed(2), close:+c.toFixed(2) });
    p = c;
  }
  return bars;
}
...
async function getQuote(ticker: string): Promise<StockQuote> {
  const mock = MOCK_QUOTES[ticker] ?? { ticker, name:ticker, price:100, change:0, changePct:0, high:101, low:99, open:100, volume:1_000_000 };
  ...
  return { ticker, name:mock.name, price:day.c, change:+change.toFixed(2), changePct:+((change/prevDay.c)*100).toFixed(2), high:day.h, low:day.l, open:day.o, volume:day.v };
}
...
async function searchTickers(q: string): Promise<string[]> {
  const data = await polyFetch<{ results?:{ticker:string;name:string}[] }>(`/v3/reference/tickers?search=${encodeURIComponent(q)}&active=true&limit=6&market=stocks`);
  if (!data?.results?.length) return POPULAR.filter(t => t.includes(q.toUpperCase()) || (MOCK_QUOTES[t]?.name.toLowerCase().includes(q.toLowerCase())));
...
export default function VertexTerminal() {
  ...
  // Load quote + bars when ticker changes (only relevant for Dashboard tab)
  ...
  const selectTicker = (t: string) => { setTicker(t); setSearch(""); setResults([]); setTab("chart"); };
  ...
  const S = {
    ...
    badge: (pos: boolean) => ({ display:"inline-flex", alignItems:"center", gap:4, background:pos?"rgba(0,255,148,0.1)":"rgba(255,59,92,0.1)", color:pos?"#00FF94":"#FF3B5C", border:`1px solid ${pos?"rgba(0,255,148,0.2)":"rgba(255,59,92,0.2)"}`, borderRadius:6, padding:"2px 8px", fontFamily:"'Courier New',Courier,monospace", fontSize:12 } as React.CSSProperties),
    btn: (active: boolean) => ({ background:active?"rgba(0,212,255,0.12)":"transparent", color:active?"#00D4FF":"#7A9BBF", border:"none", borderRadius:6, padding:"4px 12px", cursor:"pointer", fontFamily:"'Courier New',Courier,monospace", fontSize:12, transition:"all 0.15s" } as React.CSSProperties),
    input: { background:"#060B14", border:"1px solid #1E293B", borderRadius:8, color:"#E2EAF4", padding:"8px 12px 8px 36px", fontFamily:"'Courier New',Courier,monospace", fontSize:13, outline:"none", width:"100%" } as React.CSSProperties,
    tag: (color: string) => ({ fontSize:10, padding:"2px 8px", borderRadius:99, background:`${color}10`, color, border:`1px solid ${color}30`, fontFamily:"'Courier New',Courier,monospace" } as React.CSSProperties),
  };

  // ─── Full-width panels (Top15 and MyStocks take full width) ───────────────
  const isFullWidth = tab === "top15" || tab === "mystocks";

  return (
    <div style={S.page}>
      {/* ── Header ──────────────────────────────────────────────────────────── */}
      <header style={{ borderBottom:"1px solid #1E293B", background:"#0D1321", padding:"0 20px", height:52, display:"flex", alignItems:"center", gap:14 }}>
        {/* Logo */}
        ...
        <div style={{ position:"relative", flex:1, maxWidth:300 }}>
          ...
          <input style={S.input} placeholder="Search ticker…" value={search} onChange={e=>setSearch(e.target.value)} />
          {searching && <RefreshCw size={12} color="#7A9BBF" style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", animation:"spin 1s linear infinite" }} />}
          {results.length > 0 && (
        ...
        <div style={{ display:"flex", gap:5, flexWrap:"nowrap", overflow:"hidden" }}>
          {POPULAR.map(t => (
            <button key={t} onClick={()=>selectTicker(t)}
              style={{ ...S.mono, fontSize:11, padding:"3px 9px", borderRadius:6, cursor:"pointer", border:"1px solid", transition:"all 0.15s", flexShrink:0,
                background: t===ticker&&tab==="chart" ? "rgba(0,212,255,0.12)":"transparent",
                color:      t===ticker&&tab==="chart" ? "#00D4FF":"#7A9BBF",
                borderColor:t===ticker&&tab==="chart" ? "rgba(0,212,255,0.3)":"#1E293B" }}>
              {t}
            </button>
          ))}
        </div>
        ...
        <div style={{ display:"flex", gap:2, marginLeft:"auto", background:"#111E30", borderRadius:8, padding:3, flexShrink:0 }}>
          {TABS.map(t => (
            <button key={t.id} onClick={()=>setTab(t.id)}
              style={{ display:"flex", alignItems:"center", gap:5, padding:"5px 11px", borderRadius:6, border:"none", cursor:"pointer", fontSize:12, fontWeight:500, transition:"all 0.15s", whiteSpace:"nowrap",
                background: tab===t.id ? "#0D1321":"transparent",
                color:      tab===t.id ? "#E2EAF4":"#7A9BBF",
      ...
      {/* ── Two-column layout for Dashboard + AI ─────────────────────────── */}
      {!isFullWidth && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 280px", gap:20, padding:20, maxWidth:1400, margin:"0 auto" }}>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
              <div>
                <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                  ...
                  <button onClick={()=>toggleWatch(ticker)} style={{ background:"none", border:"none", cursor:"pointer", padding:4 }}>
              ...
              <div style={{ textAlign:"right" }}>
                {loading
                  ...
                  : <>
                      ...
                      <div style={{ ...S.mono, fontSize:13, ...(up?S.green:S.red) }}>{quote.change>=0?"+":""}{fmt$(quote.change)} today</div>
                    </>}
              </div>
            </div>
                  ...
                  {loading ? (
                    ...
                    <ResponsiveContainer width="100%" height={240}>
                      <AreaChart data={bars} margin={{ top:4, right:8, left:0, bottom:0 }}>
                        ...
                        <XAxis dataKey="date" tick={{ fill:"#3D5A7A", fontSize:9, fontFamily:"Courier New" }} tickLine={false} axisLine={false} interval="preserveStartEnd"/>
                        <YAxis tick={{ fill:"#3D5A7A", fontSize:9, fontFamily:"Courier New" }} tickLine={false} axisLine={false} tickFormatter={(v:number)=>`$${v.toFixed(0)}`} width={54} domain={["auto","auto"]}/>
            ...
            {tab === "ai" && (
              <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
                ...
                <div style={{ background:"rgba(255,184,0,0.05)", border:"1px solid rgba(255,184,0,0.2)", borderRadius:10, padding:"12px 14px", display:"flex", gap:10 }}>
                  <AlertTriangle size={15} color="#FFB800" style={{ marginTop:1 }}/>
                  <p style={{ ...S.muted, fontSize:11, lineHeight:1.6, margin:0 }}>
                    AI predictions are for informational purposes only. Always conduct your own research before making investment decisions.
                  </p>
                </div>
              </div>
            )}
          ...
          {/* ── Right column — Watchlist + Market Snapshot ────────────── */}
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <div style={{ ...S.card, overflow:"hidden" }}>
              ...
              {watchlist.length === 0 && <p style={{ ...S.muted, fontSize:12, textAlign:"center", padding:20 }}>Star a stock to add it here</p>}
              {watchlist.map(t => {
            ...
            <div style={{ ...S.card, padding:"14px 16px" }}>
              <p style={{ ...S.muted, fontSize:10, textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:10 }}>Quick Access</p>
              <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                <button onClick={()=>setTab("top15")}
                  style={{ display:"flex", alignItems:"center", gap:8, background:"rgba(255,184,0,0.06)", border:"1px solid rgba(255,184,0,0.2)", borderRadius:8, color:"#FFB800", padding:"8px 12px", cursor:"pointer", fontSize:12, fontWeight:500 }}>
                  <Trophy size={13}/> View Top 15 Stocks
                </button>
                <button onClick={()=>setTab("mystocks")}
                  style={{ display:"flex", alignItems:"center", gap:8, background:"rgba(168,85,247,0.06)", border:"1px solid rgba(168,85,247,0.2)", borderRadius:8, color:"#A855F7", padding:"8px 12px", cursor:"pointer", fontSize:12, fontWeight:500 }}>
                  <BookOpen size={13}/> Manage My Stocks
                </button>
              </div>
            </div>
          </div>
