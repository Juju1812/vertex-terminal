...
/* ── Mini sparkline for AI signal cards ─────────────────────── */
function MiniChart({ ticker, isGain }: { ticker: string; isGain: boolean }) {
  ...
  const gradId = `mc-${ticker}-${isGain ? "g" : "l"}`;          // unique per card

  useEffect(() => {
    let cancelled = false;
    setBars(seedBars(MOCK[ticker]?.price ?? 100, 30));            // instant seed
    setReady(false);
    loadBars(ticker).then(data => {
      if (!cancelled) { setBars(data.slice(-30)); setReady(true); }
    });
    return () => { cancelled = true; };
  }, [ticker]);                                                   // ← ticker in deps
...
function SignalCard({
  ...
  ticker:string; name:string; conf:number; target:number; upside?:number; downside?:number;
  thesis:string; tags:string[]; isGain:boolean; rank:number; go:(t:string)=>void;
}) {
  const color = isGain ? V.gain : V.loss;
  ...
  return (
    <div style={{ ...glassCard({ padding:18 }), cursor:"pointer", transition:"border-color 0.25s, transform 0.2s cubic-bezier(0.16,1,0.3,1)" }}
      onClick={() => go(ticker)}
      ...
      <div style={{ display:"flex", flexWrap:"wrap", gap:5 }}>
        {tags.map(t => (
          <span key={t} style={{ ...mono, fontSize:9, padding:"3px 9px", borderRadius:99, background:dim, color, border:`1px solid ${wire}`,
            display: isGain ? "inline" : "inline-flex", alignItems:"center", gap:3 }}>
    ...
    <div style={{ minHeight:"100vh", background:V.d0, color:V.ink1, fontFamily:"'Bricolage Grotesque',system-ui,sans-serif" }}>
      ...
      <header style={{ position:"sticky", top:0, zIndex:100, background:"rgba(5,8,16,0.94)", backdropFilter:"blur(40px) saturate(2.5)", WebkitBackdropFilter:"blur(40px) saturate(2.5)", borderBottom:`1px solid ${V.w2}` }}>
        ...
        <div style={{ display:"flex", alignItems:"center", padding:"0 16px", height:32, borderBottom:`1px solid ${V.w1}`, overflow:"hidden" }}>
          <div className="vx-strip" style={{ flex:1, display:"flex", alignItems:"center", gap:14, overflowX:"auto", scrollbarWidth:"none", WebkitOverflowScrolling:"touch" as React.CSSProperties["WebkitOverflowScrolling"] }}>
            {INDICES.map(m => (
              ...
              <div style={{ position:"fixed", right:16, top:90, width:"min(340px,calc(100vw - 32px))", background:"rgba(8,13,24,0.97)", backdropFilter:"blur(40px)", WebkitBackdropFilter:"blur(40px)", border:`1px solid ${V.w3}`, borderRadius:14, overflow:"hidden", zIndex:200, boxShadow:"0 20px 60px rgba(0,0,0,0.75)" }}>
                <div style={{ display:"flex", alignItems:"center", gap:8, padding:"11px 14px", borderBottom:`1px solid ${V.w1}` }}>
                  ...
                  <input autoFocus value={search} onChange={e=>setSearch(e.target.value)} className="vx-input" placeholder="Search ticker or company…"
                    style={{ background:"transparent", border:"none", boxShadow:"none", padding:0, borderRadius:0, fontSize:16, flex:1, color:V.ink0, outline:"none", fontFamily:"'Geist Mono',monospace" }}/>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; }
        ...
        /* Hide scrollbars on the index strip */
        ...
        /* Tab labels — hidden icon-only on mobile, text shown ≥480px */
        .tab-label { display:none; }
        @media(min-width:480px){ .tab-label { display:inline; } }

        /* Sidebar + two-col grid on large screens */
        ...
        /* ── MOBILE-SPECIFIC ── */
        /* On mobile: bottom nav visible, desktop header tabs icon-only */
        @media(max-width:767px){
          /* Main content clears the fixed bottom nav (56px) + safe-area */
          .vx-main { padding-bottom:calc(64px + env(safe-area-inset-bottom, 0px)) !important; }
          /* Reduce main horizontal padding so content doesn't crowd */
          .vx-main > div { padding-left:12px !important; padding-right:12px !important; }
